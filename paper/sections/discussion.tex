...
After the archived five-target study, we ran an additional focused B3/B5 experiment using the Codex CLI backend with \texttt{gpt-5.4} for mutation and \texttt{gpt-5.4-mini} for diagnosis and judging over 15 paired seeds.
We do not treat that run as a primary replication of the end-to-end result bundle: it excludes B1, B2, and B4, omits the raw baseline, and uses a reduced-fidelity B5 protocol (\texttt{sample\_count=1}, \texttt{judgment\_votes=1}, budget 5) to make Codex-backed stochastic judging operationally tractable.
Its main signal is methodological rather than evidential: both selected targets saturated under the reduced protocol, with all B3 configurations reaching the best observed score and B5 operating near ceiling.
The focused run therefore reinforces the need for harder benchmark variants and direction-aware best-score analysis before additional seed expansion can support stronger treatment-effect claims.
...
A focused 15-seed Codex-backed B3/B5 run showed that simply increasing seeds on the current B3/B5 definitions is not enough: both targets can saturate under reduced protocols, eliminating score headroom.
The next replication should therefore pair higher seed counts with harder target variants and direction-aware primary metrics (best score for minimization targets, success rate, and cost-to-threshold), rather than only rerunning the same saturated tasks.
