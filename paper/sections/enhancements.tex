...
Adaptive reheat prevents premature convergence: when the acceptance ratio over the last 10 experiments drops below half the \texttt{acceptance\_target} (default $0.3$), the temperature is multiplied by \texttt{reheat\_factor} (default $2.0$), capped at the initial temperature.
This mechanism allows the search to escape local optima when the landscape becomes flat.

\texttt{HybridSearch} provides a sensible default for users who do not configure a search strategy: greedy for the first 10 experiments to establish a strong baseline, then simulated annealing to explore the neighborhood.
...
Every \texttt{migration\_interval} experiments (default 10), the best individual from each island is copied to every other island, injecting diversity without destroying each island's locally adapted population.
...
Context assembly follows a cold-start protocol: for the first 10 experiments, only raw records are injected; after that threshold, the agent receives the five most recent records, the five most similar past experiments by hypothesis embedding, and a consolidated learnings summary.

Every 50 experiments, deterministic consolidation produces a \texttt{ConsolidationRecord} summarizing kept/discarded/crashed counts, score trajectories, top improvements, failed approaches, tag frequencies, per-criterion variances, verifier block rates, and failure distribution.
...
The pool evicts lowest-impact learnings when it exceeds \texttt{max\_size} (default 1000).
...
This mechanism reduces cost on clear-cut experiments while increasing statistical power on marginal ones.
...
\begin{table*}[t]
\centering
\caption{Summary of enhancements by track, mechanism, and evidence status. The
\emph{Status} column separates isolated mechanism validation from implemented
features that are not independently evaluated in the end-to-end study.}
\label{tab:enhancements}
\footnotesize
\setlength{\tabcolsep}{4pt}
\begin{tabular}{@{}p{2.4cm}p{1.7cm}p{4.1cm}p{5.4cm}p{1.8cm}@{}}
\toprule
\textbf{Name} & \textbf{Track} & \textbf{Mechanism} & \textbf{Benefit} & \textbf{Status} \\
\midrule
Strategy Manifest & Representation & Structured YAML with named components & Component-level evolution; avoids catastrophic forgetting of working strategies & implemented \\
Token-Efficient Context & Representation & Three compression modes & Reduces input token cost; keeps agent within context budget & implemented \\
Dual-Agent Mutation & Search & Thompson Sampling over Beta-Binomial arms & Adaptive explore-exploit without manual scheduling & implemented \\
Two-Phase Mutation & Search & Diagnosis step before mutation & Focuses mutations on weakest criteria and root cause & implemented \\
Simulated Annealing & Search & $\exp(\Delta/T)$ acceptance with adaptive reheat & Escapes local optima; reheat prevents premature convergence & validated \\
Island Population & Search & $N$ independent populations with migration & Maintains diversity; best individuals propagate across islands & implemented \\
Episodic Memory (TF-IDF) & Knowledge & Structured Lesson extraction + TF-IDF / sentence-embedding retrieval & Avoids repeating failed approaches; transfers successful patterns & validated \\
Lineage Context & Knowledge & Causal chain of accepted mutations & Agent understands how current artifact state was reached & implemented \\
Research Operator & Knowledge & External knowledge injection on plateau & Breaks out of exploration dead ends & implemented \\
Cross-Domain Transfer & Knowledge & Learning pool with decay and domain penalties & Transfers insights across targets and projects & implemented \\
Adaptive Sampling & Evaluation & Cohen's $d$ early-stop / extend & Reduces cost on clear experiments; increases power on marginal ones & implemented \\
Drift Detection & Evaluation & Sliding-window variance comparison & Detects evaluator degradation before it corrupts optimization & implemented \\
Eval Caching & Evaluation & Content-addressed LRU cache & Eliminates redundant evaluation of identical artifacts & implemented \\
\bottomrule
\end{tabular}
\end{table*}

% ==================================================================
\subsection{Default Hyperparameters}\label{sec:hyperparameters}

\Cref{tab:hyperparameters} consolidates the default values for every configurable hyperparameter introduced in \cref{sec:system-design,sec:enhancements}, together with the component each parameter belongs to and a one-line rationale.
Values are the defaults shipped with the engine; every entry is overridable per-target via the corresponding configuration class (e.g.\ \texttt{SearchConfig}, \texttt{LearningPoolConfig}, \texttt{StochasticEval}).

\begin{table*}[t]
\centering
\caption{Default hyperparameters across components. Values listed are the engine-shipped defaults; the corresponding configuration class for each component allows per-target override.}
\label{tab:hyperparameters}
\footnotesize
\begin{tabular}{@{}p{4.0cm}p{4.0cm}p{2.6cm}p{5.0cm}@{}}
\toprule
\textbf{Parameter} & \textbf{Component} & \textbf{Default} & \textbf{Rationale} \\
\midrule
$\alpha$ (significance level)         & Statistical Comparison (\cref{sec:stat-comparison}) & $0.05$ & Standard threshold for paired Wilcoxon test \\
\texttt{window\_size}                 & Holm--Bonferroni window (\cref{sec:stat-comparison}) & $50$ experiments & Matches consolidation cadence (\cref{sec:episodic-memory}) \\
Divergence warning threshold          & Held-out divergence (\cref{sec:stat-comparison}) & $10\%$ & Mild overfitting alert \\
Divergence critical threshold         & Held-out divergence (\cref{sec:stat-comparison}) & $25\%$ & Hard overfitting alert \\
\texttt{cooling\_rate}                & Simulated Annealing (\cref{sec:simulated-annealing}) & $0.95$ & Geometric temperature decay per experiment \\
\texttt{min\_temperature}             & Simulated Annealing (\cref{sec:simulated-annealing}) & $0.01$ & Floor to keep $\exp(\Delta/T)$ well-defined \\
\texttt{acceptance\_target}           & Adaptive Reheat (\cref{sec:simulated-annealing}) & $0.3$ & Reheat fires when ratio drops below half this value \\
\texttt{reheat\_factor}               & Adaptive Reheat (\cref{sec:simulated-annealing}) & $2.0$ & Temperature multiplier on stall, capped at $T_0$ \\
Greedy phase length                   & Hybrid Search (\cref{sec:simulated-annealing}) & $10$ experiments & Establish baseline before annealing \\
\texttt{migration\_interval}          & Island Population (\cref{sec:island-search}) & $10$ experiments & Round-robin best-individual migration cadence \\
Lineage depth $d$                     & Lineage Context (\cref{sec:lineage}) & $5$ & Last $d$ accepted ancestors injected into prompt \\
\texttt{max\_budget\_usd}             & Research Operator (\cref{sec:research-operator}) & \$$0.50$ & Per-call budget cap on plateau-recovery LLM call \\
\texttt{max\_suggestions}             & Research Operator (\cref{sec:research-operator}) & $3$ & Novel mutation directions returned per invocation \\
\texttt{disable\_after\_failures}     & Research Operator (\cref{sec:research-operator}) & $3$ & Auto-disable after consecutive unhelpful suggestions \\
$\lambda$ (decay rate)                & Learning Pool (\cref{sec:learning-pool}) & $0.05$ / day & Exponential time decay on learning impact \\
\texttt{domain\_penalty}              & Learning Pool (\cref{sec:learning-pool}) & $0.5$ & Cross-domain weight; same-domain transfer prioritized \\
\texttt{max\_size}                    & Learning Pool (\cref{sec:learning-pool}) & $1000$ & Lowest-impact eviction when exceeded \\
\texttt{early\_stop\_effect\_size}    & Adaptive Sampling (\cref{sec:adaptive-sampling}) & $1.0$ & Cohen's $d$ above which evaluation terminates early \\
\texttt{extend\_effect\_size}         & Adaptive Sampling (\cref{sec:adaptive-sampling}) & $0.3$ & Cohen's $d$ below which sampling extends \\
Sample ceiling                        & Adaptive Sampling (\cref{sec:adaptive-sampling}) & $1.5 \times$ \texttt{sample\_count} & Hard cap on extension \\
Drift variance threshold              & Drift Detection (\cref{sec:drift-detection}) & $0.1$ & Per-criterion variance flag \\
Drift variance ratio                  & Drift Detection (\cref{sec:drift-detection}) & $2\times$ & Half-window variance increase flag \\
\texttt{EvalCache} max entries        & Eval Caching (\cref{sec:eval-cache}) & $200$ & LRU bound \\
Score-history std flag                & Eval Caching (\cref{sec:eval-cache}) & $0.1$ & Cross-run consistency outlier flag \\
\texttt{stall\_timeout\_seconds}      & Agent Runtime (\cref{sec:worktree}) & $300$ & Detect silent subprocess stalls \\
\texttt{max\_transient\_retries}      & Agent Runtime (\cref{sec:worktree}) & $3$ & Bound provider retry loops \\
\texttt{transient\_retry\_base\_seconds} & Agent Runtime (\cref{sec:worktree}) & $10.0$ & Initial exponential-backoff delay \\
\texttt{transient\_retry\_cap\_seconds} & Agent Runtime (\cref{sec:worktree}) & $300.0$ & Maximum retry delay \\
\bottomrule
\end{tabular}
\end{table*}
