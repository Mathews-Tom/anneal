...
\begin{table*}[t]
\centering
\caption{Summary of enhancements by track, mechanism, and benefit. The
\emph{Val} column marks isolated mechanism validation in \cref{sec:results}:
$\checkmark$ indicates the component is one of the three mechanisms tested
against an analytically known ground truth (\cref{sec:gate-false-positives,sec:gate-sa,sec:gate-retrieval});
a dash indicates the component is argued from design and contributes to the
end-to-end comparison in \cref{sec:end-to-end} as part of the treatment
configuration's enhancement bundle.}
\label{tab:enhancements}
\footnotesize
\setlength{\tabcolsep}{4pt}
\begin{tabular}{@{}p{2.4cm}p{1.7cm}p{4.5cm}p{6.0cm}c@{}}
\toprule
\textbf{Name} & \textbf{Track} & \textbf{Mechanism} & \textbf{Benefit} & \textbf{Val} \\
\midrule
Strategy Manifest & Representation & Structured YAML with named components & Component-level evolution; avoids catastrophic forgetting of working strategies & --- \\
Token-Efficient Context & Representation & Three compression modes & Reduces input token cost; keeps agent within context budget & --- \\
Dual-Agent Mutation & Search & Thompson Sampling over Beta-Binomial arms & Adaptive explore-exploit without manual scheduling & --- \\
Two-Phase Mutation & Search & Diagnosis step before mutation & Focuses mutations on weakest criteria and root cause & --- \\
Simulated Annealing & Search & $\exp(\Delta/T)$ acceptance with adaptive reheat & Escapes local optima; reheat prevents premature convergence & $\checkmark$ \\
Island Population & Search & $N$ independent populations with migration & Maintains diversity; best individuals propagate across islands & --- \\
Episodic Memory (TF-IDF) & Knowledge & Structured Lesson extraction + TF-IDF / sentence-embedding retrieval & Avoids repeating failed approaches; transfers successful patterns; retrieval precision validated in \cref{sec:gate-retrieval} & $\checkmark$ \\
Lineage Context & Knowledge & Causal chain of accepted mutations & Agent understands how current artifact state was reached & --- \\
Research Operator & Knowledge & External knowledge injection on plateau & Breaks out of exploration dead ends & --- \\
Cross-Domain Transfer & Knowledge & Learning pool with decay and domain penalties & Transfers insights across targets and projects & --- \\
Adaptive Sampling & Evaluation & Cohen's $d$ early-stop / extend & Reduces cost on clear experiments; increases power on marginal ones & --- \\
Drift Detection & Evaluation & Sliding-window variance comparison & Detects evaluator degradation before it corrupts optimization & --- \\
Eval Caching & Evaluation & Content-addressed LRU cache & Eliminates redundant evaluation of identical artifacts & --- \\
\bottomrule
\end{tabular}
\end{table*}
