...
The strategy manifest (\texttt{StrategyManifest} in \texttt{anneal/engine/strategy.py}) replaces this with a structured YAML document containing four named components: \texttt{hypothesis\_generation}, \texttt{context\_assembly}, \texttt{mutation\_style}, and \texttt{failure\_analysis}.
Each component is a \texttt{StrategyComponent} with an \texttt{approach} field (natural-language instructions), an \texttt{evolved\_at} counter tracking the last experiment at which it was rewritten, and a \texttt{streak\_without\_improvement} counter.

Component-level evolution targets the weakest link: \texttt{StrategyManifest.weakest\_component()} returns the component with the longest streak without improvement.
The \texttt{evolve\_weakest\_component} function constructs a focused rewriting prompt that includes the current approach text, recent experiment statistics (kept/discarded counts, score ranges), and per-criterion pass/fail summaries.
...
The \texttt{AgentSelector} class (\texttt{anneal/engine/strategy\_selector.py}) maintains two Beta-Binomial arms---\texttt{primary} and \texttt{exploration}---and selects between them using Thompson Sampling.
The primary agent uses the model and temperature specified in \texttt{AgentConfig}; the exploration agent uses a separate model (\texttt{exploration\_model}) at higher temperature, biased toward novel mutations.
