...
AlphaEvolve provides evolutionary search for code but lacks statistical comparison and knowledge transfer.
...
\paragraph{Technical contributions (evaluated in \cref{sec:results}).}
...
\begin{enumerate}
  \item \textbf{Statistical acceptance under stochastic evaluation} (\cref{sec:gate-false-positives}).
    ...
    On a synthetic null distribution, Holm--Bonferroni correction reduces false-positive acceptance by \fpReductionPct\% relative to uncorrected thresholding.

  \item \textbf{Adaptive metaheuristic search} (\cref{sec:gate-sa}).
    Beyond greedy hill-climbing, \textsc{anneal} implements simulated annealing with adaptive cooling and reheat, population-based search with tournament selection and LLM-guided crossover, island migration for diversity maintenance, Pareto search for multi-objective optimization, and bandit selection among search arms.
    The isolated validation in this paper targets the adaptive-reheat mechanism rather than every implemented search mode.
    On the 3D Rastrigin function, adaptive reheat yields a directional \saImprovementPct\% improvement over fixed geometric cooling under the matched-seed protocol, narrowly missing the precommitted 15\% gate.
  ...
  \item \textbf{Structured episodic knowledge retrieval} (\cref{sec:gate-retrieval}).
    ...
    The isolated validation in this paper targets retrieval precision; on a controlled similarity corpus, TF-IDF retrieval achieves $\retrievalRatio\times$ the precision of Jaccard similarity.
\end{enumerate}
...
\begin{enumerate}
  ...
  \item \textbf{Git-native isolation and safety.}
    ...
    Budget caps (per-experiment and daily), cost tracking per model, disk space checks, process-group time-boxing, stall watchdogs, and bounded retry classification prevent runaway resource consumption.
...
Second, we run an exploratory end-to-end comparison of four benchmark configurations (raw, greedy, control, treatment) on the five-target benchmark suite under the paired-seed protocol of \cref{sec:stat-design}.
\Cref{sec:results} reports both: the mechanism gates pass their benchmark-defined thresholds, and the end-to-end study finds a consistent directional advantage of the treatment configuration over control on every target, with the framework's own multiplicity-corrected significance test declining to mark these gaps as significant at $n = 5$ paired seeds.
We therefore interpret the end-to-end run as directional systems evidence and cost telemetry, not as a causal estimate of each enhancement's independent contribution.
