...
\begin{table*}[t]
  ...
  \begin{tabular}{@{}llllll@{}}
    ...
    \midrule
    autoresearch~\cite{karpathy2025autoresearch} & P & G & --- & D & C \\
...
\textsc{anneal} captures a related idea through its strategy manifest, which decomposes the mutation approach into named components and evolves the weakest component when progress stalls, and through a policy-rewriting component (\texttt{PolicyAgent}) that periodically rewrites the mutation instructions in response to recurring failure patterns recorded in episodic memory.
...
\textsc{anneal} offers population-based search as one of several implemented strategies and includes Thompson Sampling~\cite{thompson1933likelihood} as an optional strategy-selection mechanism, rather than committing to a single evolutionary algorithm.
...
\textsc{anneal} implements Thompson Sampling as an optional meta-strategy to select among search algorithms (greedy, simulated annealing, population-based) based on each algorithm's observed performance during the optimization run, treating strategy selection as a contextual bandit problem with Beta-distributed priors updated after each experiment.
...
\textsc{anneal} targets a different level: rather than generating new metaheuristics, it exposes existing metaheuristic strategies (greedy, simulated annealing, population, Thompson Sampling meta-selection) as decision layers over LLM-generated artifact edits.
...
\textsc{anneal} uses Bradley--Terry scoring with position-bias cancellation to calibrate its LLM judge: when the number of evaluation votes is two or more, half are presented with criteria in forward order and half in reverse, and the resulting win rates are combined to estimate calibrated quality scores that cancel the position bias documented in LLM-as-judge work~\cite{zheng2023judging}.
Liu et al.~\cite{liu2023geval} extend this line by showing that calibration via chain-of-thought and probability-weighted scoring yields better alignment with human judgments than raw preference counts, motivating evaluator designs that go beyond vote tallies.
