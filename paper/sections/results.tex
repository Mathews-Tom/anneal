...
B3 is noisier: its raw mean is much worse than treatment, but its raw median is zero in the generated summary because several raw executions hit the parser's best-case floor while others are extreme slow outliers.
...
The mechanism gates and the end-to-end comparison together establish the strongest claim the data support: the validated subsystems behave as specified in isolation, and the archived treatment configuration produces a consistent directional advantage over greedy hill-climbing and over control on every target in the shipped suite.
...
We treat this conservatism as a feature: the same machinery that produced the 86\% false-positive reduction in \cref{sec:gate-false-positives} produces a stricter standard for systemic claims here, and the directional pattern across targets together with the strongest B3 and B5 greedy-versus-treatment effects ($d = -1.09$ and $d = 1.35$, $p_{\text{corr}} = 0.312$) identifies the comparisons a future higher-powered replication should prioritize.
