...
\begin{enumerate}
    ...
    \texttt{AgentInvoker} dispatches to either a Claude Code subprocess (which writes files directly to disk) or a direct API call depending on \texttt{AgentConfig.mode}; in API mode, the runner extracts the returned fenced code block and writes it to the registered artifact path before scope enforcement.
\end{enumerate}
...
\State \Call{VerifyScopeHash}{$\mathcal{T}$.\textit{scope\_path}, $\mathcal{T}$.\textit{scope\_hash}} \Comment{halt if SHA-256 of \texttt{scope.yaml} differs from registration}
...
\If{$\textit{mutation}$ is timeout, stall, or structural failure}
...
Agent invocation is not treated as an infallible atomic call.
Claude Code subprocesses run under process-group time limits and stall watchdogs; transient provider failures are retried with bounded exponential backoff, while structural failures such as invalid output format or scope violations are classified and recorded without retrying indefinitely.
